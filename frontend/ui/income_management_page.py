"""Income Management — income source groups and line entries."""
...
class _IncomeDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        self._head.setPlaceholderText("e.g. Books, Uniform, Stationary")
        ...
        if income_date is not None:
            self._date.setDate(QDate(income_date.year, income_date.month, income_date.day))
        self._particulars = QLineEdit((income_particulars or "").strip())
    ...
    def payload(self) -> dict:
        ...
        return {
            ...
            "income_date": date(qd.year(), qd.month(), qd.day()),
...
class _EntryDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        form = QFormLayout()
        form.setSpacing(14)
        self._income_source = QComboBox()
        self._income_source.setMinimumHeight(40)
        for income in service.list_incomes():
            self._income_source.addItem(str(income.head or "").strip(), int(income.id))
        if income_id is not None:
            idx = self._income_source.findData(int(income_id))
            if idx >= 0:
                self._income_source.setCurrentIndex(idx)
        ...
        form.addRow("Income source", self._income_source)
        form.addRow("Date", self._date)
    ...
    def payload(self) -> dict:
        qd = self._date.date()
        return {
            "income_id": int(self._income_source.currentData()),
...
class _ExportFilterDialog(QDialog):
    ...
    def __init__(self, service: MiscIncomeService, parent=None):
        super().__init__(parent)
        self._expense_count = 0
        self.setWindowTitle("Export income")
        self.setMinimumSize(self._DIALOG_WIDTH, self._DIALOG_HEIGHT)
        self.resize(self._DIALOG_WIDTH, self._DIALOG_HEIGHT)
        theme.apply_dialog_theme(self)
        ...
        hint = QLabel(
            "Combine date filters with income source selection. Choose a specific month or a date range "
            "(not both). Pick all income sources, one source, or any group from the checklist."
        )
        ...
        expense_group = QGroupBox("Income sources to include")
        expense_lay = QVBoxLayout(expense_group)
        expense_lay.setSpacing(10)

        mode_row = QHBoxLayout()
        self._expense_all = QRadioButton("All income sources")
        self._expense_pick = QRadioButton("Selected income sources")
        ...
        self._expense_search.setPlaceholderText("Search income sources…")
        ...
        for expense in service.list_incomes():
        ...
        if self._expense_count == 0:
            empty = QLabel("No income sources recorded yet.")
        ...
        self._expense_list.itemChanged.connect(lambda _: self._update_income_summary())
    ...
    def _selected_income_ids(self) -> list[int]:
        ids: list[int] = []
        for row in range(self._expense_list.count()):
            item = self._expense_list.item(row)
            if item is None:
    ...
    def _update_income_summary(self) -> None:
        total = self._expense_count
        selected = len(self._selected_income_ids())
        if total == 0:
            self._expense_summary.setText("No income sources available")
            return
        if self._expense_all.isChecked():
            self._expense_summary.setText(f"All {total} income sources")
            return
    ...
    def payload(self) -> dict:
        ...
        if self._expense_pick.isChecked():
            income_ids = self._selected_income_ids()
            if not income_ids:
                raise ValueError("Select at least one income source to export.")
            if len(income_ids) < self._expense_count:
                filters["income_ids"] = income_ids
    ...
    def suggested_filename(self) -> str:
        ...
        if self._expense_pick.isChecked():
            ...
            if len(labels) == 1:
                ...
                parts.append(f"{len(labels)}-income-sources")
        if len(parts) == 1:
            return "income-management.xlsx"
...
class IncomeManagementPage(QWidget):
    ...
    def __init__(self, service: MiscIncomeService, parent=None, *, on_data_changed=None):
        ...
        hint = QLabel(
            "Record miscellaneous school income. Each income source has a head (e.g. Books, Uniform, Stationary), "
        ...
        self._wrapped = wrap_page(
            "Income Management",
            breadcrumb_trail("Expenses", "Income Management"),
            body,
        )
        ...
        self._add_new_income_btn.clicked.connect(self._on_add_new_income)
    ...
    def reload(self, *, reset_page: bool = False) -> None:
        ...
        total = self._service.total_received()
    ...
    def _render_page(self) -> None:
        ...
        for i, row in enumerate(rows):
            d = row.get("income_date")
            date_text = f"{d.day:02d}/{d.month:02d}/{d.year}" if d else ""
            is_reverted = bool(row.get("is_reverted", False))
            status = str(row.get("status") or ("Income reverted" if is_reverted else "Recorded"))
    ...
    def _on_add_new_income(self) -> None:
        dialog = _IncomeDialog(title="Add new income source", parent=self)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        payload = dialog.payload()
        try:
            self._service.add_new_income(
                payload["head"],
                payload["income_date"],
                notes=payload["notes"],
            )
        except ValueError as exc:
            theme.message_warning(self._window(), "Invalid income source", str(exc))
            return
        except Exception as exc:
            theme.message_critical(self._window(), "Save failed", str(exc))
            return
        theme.message_information(self._window(), "Saved", "New income source added.")
        self.reload(reset_page=True)
        self._notify_data_changed()

    def _on_add_entry(self) -> None:
        if not self._service.list_incomes():
            theme.message_warning(
                self._window(),
                "No income sources",
                "Add a new income source before recording entries.",
            )
            return
        ...
        try:
            ...
            theme.message_critical(self._window(), "Save failed", str(exc))
            return
        theme.message_information(self._window(), "Saved", "Income entry saved.")
    ...
    def _on_delete_entry(self, entry_id: int) -> None:
        reply = theme.message_question(
            ...
            "The row will stay in history with status “Income reverted”.",
        ...
        theme.message_information(
            self._window(),
            "Income reverted",
            "The income entry has been reverted and marked as “Income reverted”.",
        )
        self.reload(reset_page=False)
        self._notify_data_changed()
    ...
    def _on_export_excel(self) -> None:
        ...
        if match_count == 0:
            ...
            theme.message_warning(
                self._window(),
                "No matching entries",
                "No income entries match the selected filters." + span,
            )
            return
