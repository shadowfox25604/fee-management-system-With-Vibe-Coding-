...
from app.core.fee_control_constants import FIXED_CLASS_KEYS, normalize_class_name
from app.models import ClassSchoolFee, FeeHead, Invoice, Student
...
class ClassFeeRepository:
    ...
    def get_stored_amount(self, class_key: str) -> float | None:
        row = self.session.get(ClassSchoolFee, class_key)
        if row is None:
            return None
        return float(row.amount)

    def upsert_stored_amount(self, class_key: str, amount: float) -> None:
        row = self.session.get(ClassSchoolFee, class_key)
        if row is None:
            self.session.add(ClassSchoolFee(class_key=class_key, amount=float(amount)))
        else:
    ...
    def apply_class_school_fee(self, class_key: str, new_amount: float) -> int:
        ...
        for st in students:
            old_tariff = float(st.school_fees or 0.0)
            self._adjust_school_invoices(st, old_tariff, new_amount)
            st.school_fees = new_amount

        self.upsert_stored_amount(class_key, new_amount)
        self.session.commit()
        return len(students)
